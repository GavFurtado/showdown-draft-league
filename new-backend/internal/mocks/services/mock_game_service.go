...
func (m *MockGameService) ReportGameResult(gameID uuid.UUID, dto *requests.ReportGameRequestDTO) error {
	args := m.Called(gameID, dto)
	return args.Error(0)
}

func (m *MockGameService) FinalizeGameResult(gameID uuid.UUID, dto *requests.FinalizeGameRequestDTO) error {
	args := m.Called(gameID, dto)
	return args.Error(0)
}
