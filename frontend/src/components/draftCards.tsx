...
export default function PokemonCard({ pokemon, cost, onImageError, leaguePokemonId, addPokemonToWishlist, removePokemonFromWishlist, isPokemonInWishlist, isFlipped, onFlip, isDraftable, onDraft, isAvailable, isMyTurn, viewMode = 'draftboard', cardSize = 'default' }: DraftCardProps) {
    ...
    return (
        <div
            className={`group rounded-lg shadow-lg relative cursor-pointer [perspective:1000px] ${cardSize === 'small' ? 'h-50 w-35' : 'h-70 w-47'}`}
            onClick={handleFlip}
        >
            <div
                className={`
                ...
                <div className={`absolute inset-0 bg-background-surface rounded-lg p-4 flex flex-col items-center justify-center [backface-visibility:hidden] ${!isAvailable ? 'opacity-50 grayscale' : ''}`}>
                    <div className="relative w-full h-[100%]">
                        ...
                        <div className='flex w-[100%] justify-between'>
                            ...
                            {viewMode === 'draftboard' && isAvailable && ( // Only render buttons if available and in draftboard view
